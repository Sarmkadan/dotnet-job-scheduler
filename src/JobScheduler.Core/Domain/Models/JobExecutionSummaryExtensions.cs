...
public static class JobExecutionSummaryExtensions
{
    ...
    public static double GetFailureRate(this JobExecutionSummary summary)
    {
        ...
        return summary.TotalExecutions == 0
            ? 0
            : (double)summary.FailureCount / summary.TotalExecutions * 100;
    }
    ...
    public static double GetTimeoutCancelledRate(this JobExecutionSummary summary)
    {
        ...
        return summary.TotalExecutions == 0
            ? 0
            : (double)(summary.TimedOutCount + summary.CancelledCount) / summary.TotalExecutions * 100;
    }
    ...
    /// <summary>
    /// Calculates the standard deviation of execution durations using a range-based approximation.
    /// Uses the formula: stdDev ≈ (max - min) / 6 for normal distributions.
    /// </summary>
    /// <param name="summary">The job execution summary to analyze.</param>
    /// <exception cref="ArgumentNullException"><paramref name="summary"/> is <see langword="null"/>.</exception>
