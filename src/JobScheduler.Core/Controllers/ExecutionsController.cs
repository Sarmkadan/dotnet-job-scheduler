...
/// <summary>
    ...
    public sealed class ExecutionDetailsResponse
    {
        ...
        /// <summary>
        /// Returns a string that summarizes the key properties of the execution details.
        /// </summary>
        public override string ToString() =>
            $"Execution {Id} for job {JobName} ({JobId}) - Status: {Status}, " +
            $"ExecutionTimeMs: {ExecutionTimeMs}, RetryAttempt: {RetryAttempt}/{MaxRetries}";
    }
...
public sealed class ExecutionStatsResponse
{
    ...
    /// <summary>
    /// Returns a string that summarizes the key properties of the execution statistics.
    /// </summary>
    public override string ToString() =>
        $"Job {JobId} - Total: {TotalExecutions}, Successful: {SuccessfulExecutions}, " +
        $"Failed: {FailedExecutions}, SuccessRate: {SuccessRate:P}, " +
        $"AvgExecutionTimeMs: {AverageExecutionTimeMs}, LastExecutionAt: {LastExecutionAt}";
}
...
public sealed class PerformanceAnalysisResponse
{
    ...
    /// <summary>
    /// Returns a string that summarizes the key properties of the performance analysis.
    /// </summary>
    public override string ToString() =>
        $"Job {JobId} - AvgExecutionTimeMs: {AverageExecutionTimeMs}, " +
        $"Median: {MedianExecutionTimeMs}, P95: {P95ExecutionTimeMs}, P99: {P99ExecutionTimeMs}, " +
        $"Slowest: {SlowestExecutionTimeMs} (at {SlowestExecutionAt}), " +
        $"Fastest: {FastestExecutionTimeMs} (at {FastestExecutionAt})";
}
...
public sealed class CleanupResponse
{
    ...
    /// <summary>
    /// Returns a string that summarizes the key properties of the cleanup response.
    /// </summary>
    public override string ToString() =>
        $"DeletedCount: {DeletedCount}, CutoffDate: {CutoffDate}, Message: {Message}";
}
